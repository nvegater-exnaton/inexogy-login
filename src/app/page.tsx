...
interface OAuthApiErrorResponse {
  error: string;
}
...
function HomeContent() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const apiUrl = `/api/oauth?oauth_token=${oauthToken}&email=${encodeURIComponent(email)}&password=${encodePassword(password)}`;
      const response = await fetch(apiUrl, {
        method: 'GET',
      });

      const result: OAuthApiResponse | OAuthApiErrorResponse =
        await response.json();

      if (!response.ok) {
        const errorResult = result as OAuthApiErrorResponse;
        throw new Error(errorResult.error || 'API request failed');
      }

      // Process response from our API route
      const successResult = result as OAuthApiResponse;
      const isRedirect =
        successResult.status >= 300 && successResult.status < 400;
      const responseBody = JSON.stringify(successResult.body);

      const oauthVerifier = successResult.body.oauthVerifier;
